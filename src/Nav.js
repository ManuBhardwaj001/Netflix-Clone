...
function Nav({ isAccountScreen = false, isHomeScreen = false, isPlanScreen }) {
    ...
    <>
      <div
        className={`nav ${navBlack ? "nav__black" : ""} ${
          isAccountScreen ? "nav__white" : ""
        }`}
      >
        <div className="nav__contents">
          <img
            className={`nav__logo ${isAccountScreen && "white__logo"}`}
            onClick={() => {
              navigate("/");
            }}
      ...
      {!isHomeScreen ? <div className="hiddenDiv"></div> : ""}
    </>
  );
}
